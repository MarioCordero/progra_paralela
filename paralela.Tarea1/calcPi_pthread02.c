...
#define MAX_THREADS 5000 //Max amount of threads
...
void* throw_darts(void* arg) {
  ...
  for (int i = 0; i < points_per_thread; i++) {
	...
	// PROTEGER LA VARIABLE CON UN MUTEX
    if (sqrt(x * x + y * y) <= 1) {
		// Proteger el mutex antes de modificar points_inside_circle
		pthread_mutex_lock(&mutex);
			points_inside_circle++;
		// Desbloquear el mutex después de modificar points_inside_circle
